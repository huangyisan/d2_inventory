...
OUTPUT = os.path.join(HERE, "暗黑2工具箱.html")
...
def main():
    ...
    itemdrops_path = os.path.join(DATA_DIR, "itemdrops.json")
    if not os.path.exists(itemdrops_path):
        subprocess.check_call([sys.executable, os.path.join(HERE, "make_itemdrops.py")])
